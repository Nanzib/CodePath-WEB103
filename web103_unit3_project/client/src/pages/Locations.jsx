...
const Locations = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // safely set venue names (only if enough items)
        setVenueNames({
          venue1: arr[0]?.name || 'Venue 1',
          venue2: arr[1]?.name || 'Venue 2',
          venue3: arr[2]?.name || 'Venue 3',
          venue4: arr[3]?.name || 'Venue 4'
        })
  ...
  const setListeners = () => {
    // Use event delegation or re-query the polygons
    ...
    polygons.forEach(element => {
      ...
      // store listeners on the element so they can be removed later if needed
      element._show = show
      element._hide = hide
    })
  }
  ...
  useEffect(() => {
    return () => {
      const polygons = document.querySelectorAll('polygon')
      polygons.forEach(el => {
        if (el._show) el.removeEventListener('mouseover', el._show)
        if (el._hide) el.removeEventListener('mouseleave', el._hide)
      })
    }
  }, [])
  ...
  return (
    <div className='available-locations'>
      ...
      <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
           ...
           viewBox="0 0 1000.32 500" xmlSpace="preserve">
        ...
        <a href='/pavilion'>
          <polygon id="venue3" name='venue3' value={3} points="998.06,83.81 952.65,31.16 914.45,16.71 877.29,43.55 833.94,102.39 811.74,161.23 
          796.77,241.23 802.97,303.16 833.94,353.23 871.61,385.23 954.71,385.23 1000.32,387.81 " />
        </a>

        <a href='/americanairlines'>
          <polygon id="venue4" name='venue4' value={4} points="625,291 615,305 608,318 625,338 637,354 622.5,358 673,363.5 751,363.5 793,363.5 
